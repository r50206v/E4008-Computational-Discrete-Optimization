...
#transCosts[i][j] = cost per thousand unit shipment from plant j to warehouse i
...
# is closed
#p's row of transport <= capacity of p-th plant * binary variable if p is open or not
m.addConstrs(
...
# Demand constraints
#sum over columns w = demand of w-th warehouse
m.addConstrs(
...
# First, open all plants - this would be a feasible solution
for p in plants:
    open[p].start = 1.0
